...
with open('pan20-author-profiling/train/en/truth.txt', "r") as r:
    data = r.read().split("\n")
...
pathlist = Path('pan20-author-profiling/train/en/').glob('**/*.xml')
for path in pathlist:  # iterate files
    ...
    for child in root:
        ...
        with open('pan20-author-profiling/texts/train/en/' + spread + '/' + author + '.txt', 'w') as f:
            xi = []
            for ch in child:
                xi.append(ch.text)
            content = ' '.join(xi)
